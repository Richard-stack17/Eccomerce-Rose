...
<html class="scroll-smooth">
  ...
  <?php helper('form');?> <!-- LLamamos a la funcion helper para poder utilizarlo mas tarde -->
    <nav>
    ...
    <form class="text-black p-5 flex flex-col gap-5 max-w-[500px] mx-auto my-10" method='POST' enctype="multipart/form-data" action='<?= base_url('/guardar_producto') ?>'>
    <div>
        <h1 class="text-center text-[2rem] font-bold pb-[3px] mb-0">Registrar producto</h1>
        <div class="h-[2px] bg-black p-0 m-0 w-[80px] mx-auto my-0"></div>
    </div>
    <div>
        <input type="hidden" name='id' value='<?php foreach($resultado3['producto']->getResultArray() as $producto){
                echo $producto['ProductID'];
                }?>'>
        <label class="block font-extrabold mb-1">Nombre</label>
        <input
            type="text"
            class="block border-[2px] border-gray-400 w-full p-2"
            name='nombre'
            value='<?php foreach($resultado3['producto']->getResultArray() as $producto){
                echo $producto['ProductName'];
                }?>'
        />
    </div>
    <div>
        <label class="block font-extrabold mb-1" >Categoría</label>
        <select  name="categories" id="category" class="block border-[2px] border-gray-400 w-full p-2">
          <?php foreach($resultado['categoria']->getResultArray() as $fila){?>
          <option value="<?=$fila['CategoryID']?>"><?=$fila['CategoryName']?></option>
          <?php } ?>
        </select>
      </div>
      <div>
        <label class="block font-extrabold mb-1" >Stock</label>
        <input
          type="number"
          class="block border-[2px] border-gray-400 w-full p-2"
          name='stock'
      ...
      <div>
        <label class="block font-extrabold mb-1">Precio</label>
        <input
          type="number"
          class="block border-[2px] border-gray-400 w-full p-2"
          name='precio'
      ...
      <div>
        <label class="block font-extrabold mb-1">Proveedor</label>
        <select name="suppliers" id="category" class="block border-[2px] border-gray-400 w-full p-2">
        <?php foreach($resultado2['supplier']->getResultArray() as $fila){?>
          <option value="<?=$fila['SupplierID']?>"><?=$fila['SupplierName']?></option>
          <?php } ?>
        </select>
      </div>
      <div>
        <label class="block font-extrabold mb-1">Descripción</label>
        <textarea
          type="password"
          class="block border-[2px] border-gray-400 w-full p-2"
          name='descripcion'
        ><?php foreach($resultado3['producto']->getResultArray() as $producto){
                echo $producto['Description'];
                }?></textarea>
      </div>
    <div>
        <label class="block font-extrabold mb-1">Imagen</label>
        <input
            type="file"
            class="block border-[2px] border-gray-400 w-full p-2"
    ...
    <div>
        <button class="block text-white bg-black p-2 rounded-[4px] w-full" type="submit">GUARDAR</button>
    </div>
</form>
