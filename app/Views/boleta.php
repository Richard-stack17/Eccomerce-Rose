...
<html class="scroll-smooth">
  ...
  <body style="font-family: 'Barlow', sans-serif">
    ...
    <h1 class="text-[2.5rem] font-bold text-center my-7 text-rose-900">
      ¡Muchas gracias por tu compra :)!
    </h1>
    <div class="bg-rose-200 py-10 shadow-lg">
      <section
        class="grid grid-rows-6 justify-left items-center max-w-[700px] mx-auto pb-5 gap-4 border-[4px] rounded-lg border-black bg-white"
      >
        <h2 style="text-shadow:1.5px 1.5px black" class="font-bold text-center text-[2rem] bg-rose-600 text-white w-full">Detalles de tu compra realizada</h2>
        
        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Nombre del cliente:</p>
          <p class="font-bold text-rose-700"><span><?=$customer['CustomerName']?></span> <span><?=$customer['Last_Name']?></span></p>
        </div>
        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Email:</p>
          <p class="font-bold text-rose-700"><?=$customer['email']?></p>
        </div>

        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Nombre del producto comprado:</p>
          <p class="font-bold text-rose-700"><?=$product['ProductName']?></p>
        </div>

        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Precio del producto:</p>
          <p class="font-bold text-rose-700">S/ <?=$product['Price']?></p>
        </div>

        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Cantidad:</p>
          <p class="font-bold text-rose-700"><?=$cantidad?></p>
        </div>
  

        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Nombre del empleado que le atendió:</p>
          <p class="font-bold text-rose-700"><span><?=$employee['FirstName']?></span> <span><?=$employee['LastName']?></span></p>
        </div>
        <div class="flex justify-between gap-5 px-3 text-[1.5rem] font-bold">
          <p>Expedidor:</p>
          <p class="font-bold text-rose-700"><?=$shipper['ShipperName']?></p>
        </div>
        <div class="flex justify-between px-3 text-[1.6rem] mt-4 pt-5">
          <p class="font-black text-rose-700 ">Fecha:</p>
          <p class="font-bold text-red-500"><?php echo date('d/m/Y');?></p>
        </div>
        <div class="flex justify-between px-3 text-[1.6rem]">
          <p class="font-black  text-rose-700">Total pagado:</p>
          <p class="font-bold text-red-500">S/ <?=$totalCost?></p>
        </div>
        

      </section>
      <section style="text-shadow:1px 1.5px black" class="flex justify-center items-center my-10"><a href="<?=base_url('/')?>" class="block px-5 py-4 rounded-full bg-gradient-to-r from-rose-600 to-red-600 text-center text-white text-[1.6rem] hover:from-rose-400 hover:to-red-400  transition-colors delay-1000 hover:border-[3px] hover:border-black">Ir al inicio</a></section>
    </div>
