...
<html class="scroll-smooth">
  ...
  <body style="font-family: 'Barlow', sans-serif">
   <nav>
    ...
    <h1 class="text-[2rem] font-bold text-center text-red-700">Carrito de compras</h1>
    <form
      class="p-3 flex flex-wrap gap-5 bg-rose-100  text-[1.2rem] min-[500px]:items-center min-[500px]:justify-center text-red-700" action="<?= base_url('carrito?id=' . $product['ProductID'].'&cantidad=' . $cantidad) ?>" method="post"
    >
      <div class="flex flex-col gap-7">
        <div
          class="flex flex-col gap-4 w-full max-w-[800px] min-[500px]:flex-row " 
        >
          <div>
            <?php $rutaImagen = '../src/Imagenes/' . $product['Image'];?>
            <img src="<?= $rutaImagen ?>" alt="Imagen del producto" class="w-full max-w-[70px] mx-auto my-0">
            <p class="flex justify-between md:hidden md:flex-col">
          ...
          <div class="flex flex-col md:flex-row md:gap-10">
            ...
            <p
              class="flex justify-between min-[500px]:flex-col min-[500px]:justify-start"
            >
              <span class="font-bold">Precio:</span><span>S/. <?=$product['Price'];?></span>
            </p>
            <p
              class="hidden md:flex justify-between min-[500px]:flex-col min-[500px]:justify-start"
            >
              <span class="font-bold">Cantidad:</span><span><?=$cantidad?></span>
            </p>
          </div>
        
      <div
        style="text-shadow:1px 1px black" class="flex flex-col justify-center items-center p-3 gap-4 w-full max-w-[300px] mx-auto my-0 bg-rose-400 border-[4px] rounded-lg border-rose-400 text-white"
      >
        <h2 class="font-bold">Resumen de la compra</h2>
        <div class="flex justify-between gap-5">
          <p>Total:</p>
          <p class="font-bold text-rose-200">S/. <?=+$product['Price']*$cantidad?></p>
        </div>
        <a>
          <button class="block my-5 text-white bg-pink-900 px-4 w-full border-white border-[0.1px]" type="submit">
            Ordenar
          </button>
        </a>
    ...
    <footer
      class="bg-black text-white text-justify p-5 grid gap-5 min-[800px]:grid-cols-2 min-[800px]:grid-rows-2 min-[1100px]:grid-cols-4 min-[1100px]:grid-rows-1 min-h-[370px]"
    >
