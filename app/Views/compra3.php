...
<html class="scroll-smooth">
  ...
  <body style="font-family: 'Barlow', sans-serif">
    ...
    <h1 class="text-[2rem] font-bold text-center text-rose-900">Ya casi finalizamos la compra :)</h1>
    <section
      class="p-3 flex flex-wrap gap-5 bg-rose-200 text-[1.2rem] min-[500px]:items-center min-[500px]:justify-center"
    >
    <form class="text-rose-900 p-4 flex flex-col gap-5 max-w-[900px] mx-auto my-10" method="POST" action='<?= base_url('/generar_orden?id='.$product['ProductID'].'&cantidad='.$cantidad.'&customerId='.$customerId)?>'>
 
    <div>
            <h1 class="text-center text-[2rem] font-bold pb-[3px] mb-0">Defina su envío</h1>
            <div class="h-[2px] bg-rose-900 p-0 m-0 lg:w-[600px] mx-auto my-0 max-w-[500px]"></div>
          </div>
      <div>
         <label class="block font-extrabold mb-1">Empleado que lo atendió</label>
        <select name="empleado" id="employee" class="block border-[2px] border-gray-400 w-full p-2">
        <?php foreach($resultado3['employee']->getResultArray() as $fila){?>
          <option value="<?=$fila['EmployeeID']?>"><?=$fila['LastName']?></option>
          <?php } ?>
        </select>
      </div>
      <div>
         <label class="block font-extrabold mb-1">Enviador</label>
        <select name="enviador" id="enviador" class="block border-[2px] border-gray-400 w-full p-2">
        <?php foreach($resultado4['shipper']->getResultArray() as $fila){?>
          <option value="<?=$fila['ShipperID']?>"><?=$fila['ShipperName']?></option>
          <?php } ?>
        </select>
      </div>
      
      <a>
        <button class="block my-5 text-white bg-rose-800 p-4 w-full" type="submit">
          Generar una orden
        </button>
      </a>
    </form>
      ...
      <div
      class="flex flex-col justify-center items-center gap-4 max-w-[300px] mx-auto my-0 p-5 text-[white] border-[4px] rounded-lg border-black bg-rose-500 w-full p-0 m-0"
      >
        <h2 class="font-bold">Resumen de la compra</h2>
        <div class="flex justify-between gap-5">
          <p>Total:</p>
          <p class="font-bold ">S/ 253.00</p>
        </div>
